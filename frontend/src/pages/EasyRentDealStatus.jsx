...
export default function EasyRentDealStatus() {
  ...
  const { property, propertyFetched, dealStatus, error, baseUrl, defaultImage } = usePropertyDetails(propertyId, token, user?.id || user?._id);
  ...
  const {
    ...
    progress,
    dealError,
  } = useDeal({ property, propertyId, userId, token: stableToken, dealStatus, propertyFetched });

  const renter = {
    name: user?.name || 'Fikiresilase',
    avatar: (user?.name || 'Fikiresilase')[0].toUpperCase(), // First letter of renter's name
  };
  const owner = {
    name: property?.ownerId?.name || 'Fikiresilase',
    avatar: (property?.ownerId?.name || 'Fikiresilase')[0].toUpperCase(), // First letter of owner's name
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center px-2 sm:px-4 animate-fade-in">
      ...
      <main className="flex flex-col items-center py-6 sm:py-8 lg:py-12 w-full max-w-5xl">
        <LoadingSpinner visible={!propertyFetched} />
